...
class LinearModel(object):
    def __init__(self, state_size, input_size, learning_rate):
        ...
        self.A = np.random.randn(state_size, state_size)
...
if __name__ == "__main__":
    ...
    ax.set_ylabel('Imaginary', fontsize=25)
    ax.grid()
    fig_eign.suptitle('Eigen values of A', fontsize=30)
    ...
    ax2[2].set_xlabel('Time steps', fontsize=25)
    ax2[0].set_ylabel(r'$\theta (rad)$', fontsize=25)
    ax2[1].set_ylabel(r'$\phi (rad)$', fontsize=25)
    ax2[2].set_ylabel(r'$\beta (rad)$', fontsize=25)
    ax2[0].grid()
    ax2[1].grid()
    ax2[2].grid()

    fig_response.suptitle('Attitude Response', fontsize=30)
